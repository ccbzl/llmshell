...
class LLMShell:
    ...
    def __init__(self, model_provider: str = "ollama", model: str = "qwen3:4b", api_key: Optional[str] = None):
        """
        初始化 LLM Shell
        ...
        try:
            ...
            # 使用单例模式，异步逻辑已封装在 Manager 内部
            self.server_manager = MCPServerManager()
            # 加载MCP server配置
            self.mcp_servers = self._load_mcp_servers()
            # 懒加载模式：首次使用时自动连接，不需要预连接
    ...
    def _create_model_interface(self) -> BaseModelInterface:
        ...
        if self.model_provider == "kimi":
            ...
            return KimiInterface(model=self.model, api_key=api_key)
        elif self.model_provider == "moonshot":
            # 优先使用传入的 API key，否则会在 MoonshotInterface 中从环境变量读取
            api_key = self.api_key or None
            return MoonshotInterface(model=self.model, api_key=api_key)
        else:  # 默认使用 Ollama
    ...
    def _load_system_prompt(self) -> str:
        ...
        # print(f"[Debug] System prompt preview: {system_prompt[:500]}...")
    ...
    def _mcp_tool_list(self, server_name=None):
        """列出MCP工具"""
        if self.server_manager:
