...
│   └── tools.py        # 工具类实现
├── tests/              # 测试文件目录
│   ├── test_kimi.py    # Kimi 模型测试
│   └── test_toolcalls.py # 工具调用测试
├── scripts/            # 脚本目录
│   └── check_prompt_length.py # 提示词长度检查
├── backups/            # 备份文件目录
├── cli.js.map          # Claude Code 源码映射文件
├── llmshell.py         # 原始单文件版本
└── README.md           # 项目文档
```
...
1. 在 `llmshell/tools.py` 中创建新的工具类，继承自 `BaseTool`
2. 实现 `get_schema`、`validate_parameters` 和 `execute` 方法
3. 在 `LLMShell.__init__` 方法中注册新工具

### 新增模型

1. 在 `llmshell/models.py` 中创建新的模型接口类，继承自 `BaseModelInterface`
