...
class KimiInterface(BaseModelInterface):
    """Kimi 模型接口"""
    
    def __init__(self, model: str = "kimi-k2.5", api_key: str = None):
        self.model = model
        # 优先从环境变量读取 API key
        self.api_key = api_key or os.environ.get("KIMI_API_KEY") or os.environ.get("MOONSHOT_API_KEY")
        if not self.api_key:
            raise ValueError("Kimi API key not provided. Please set KIMI_API_KEY or MOONSHOT_API_KEY environment variable.")
    ...
    def generate(self, prompt: str, system_prompt: Optional[str] = None, stream: bool = True) -> Generator[Dict[str, Any], None, None]:
        ...
        logger.info(f"[Model Request] Sending to {self.model} (Kimi)")
        logger.debug(f"[Prompt] {prompt[:100]}...")
        if system_prompt:
            logger.debug(f"[System Prompt] {system_prompt[:100]}...")
        ...
        try:
            ...
            response.raise_for_status()
            logger.info(f"[Model Response] Received from {self.model} (Kimi)")
            ...
            error_msg = "Cannot connect to Kimi. Please make sure the API endpoint is reachable."
            logger.error(f"[Model Error] {error_msg}")
            yield {"error": error_msg}
        except Exception as e:
            error_msg = str(e)
...
class MoonshotInterface(BaseModelInterface):
    ...
    def __init__(self, model: str = "moonshot-v1-8k", api_key: str = None):
        self.model = model
        # 优先从环境变量读取 API key
        self.api_key = api_key or os.environ.get("MOONSHOT_API_KEY") or os.environ.get("KIMI_API_KEY")
        if not self.api_key:
            raise ValueError("Moonshot API key not provided. Please set MOONSHOT_API_KEY or KIMI_API_KEY environment variable.")
        self.base_url = "https://api.moonshot.cn/v1"
